...
-destruct y.
  --simpl;reflexivity.
  --simpl. intros. inversion H. destruct a, b; simpl; rewrite IHx; try apply H1; try reflexivity. 
Qed.

Lemma ALU_One (n : nat) (x y : list B) :
  length x = n /\ length y = n /\ n <> 0 -> ALU x y I I I I I I = repeat O (pred n) ++ [I].
Proof.
intros. destruct H. destruct H0. simpl. rewrite H0, H.
...
assert (P1 : forall (b : B) (n : nat), repeat b n ++ [b] = b :: repeat b n).
{
-intros. induction n0.
  --reflexivity.
  --simpl. rewrite IHn0. reflexivity.
...
assert (P2 : forall (b : B) (n : nat), rev (repeat b n) = repeat b n).
{
-intros. induction n0.
  --reflexivity.
  --simpl. rewrite IHn0. rewrite P1. reflexivity.
}

assert (PS: forall (n : nat), I :: repeat I n = repeat I (S n)).
{
-simpl. intros. reflexivity.
}
...
-intros. simpl. induction n.
  --simpl. contradiction.
  --simpl. rewrite PS. unfold AddL. rewrite P2. simpl. rewrite P_AddLr. rewrite P2. reflexivity.   
}

assert (P_NotL2: forall (n : nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
{
...
-induction n.
  --contradiction.
  --rewrite P_NotL. simpl. rewrite PS. rewrite P_AddL. simpl. rewrite P_NotL2. reflexivity. 
Qed.
